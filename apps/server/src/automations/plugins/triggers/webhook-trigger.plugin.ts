...
export class WebhookTriggerPlugin extends BaseTriggerPlugin {
  ...
  public async evaluate(config: TriggerConfig, context: TriggerContext): Promise<TriggerResult> {
    ...
    const allowedSources = this.getConfigValue(config, 'allowedSources', []);
    ...
    return {
      ...
      properties: {
        ...
        allowedSources: {
          ...
          description: 'List of allowed source identifiers (empty = allow all)',
          items: {
            type: 'string'
          },
          default: []
        ...
        payloadConditions: {
          ...
          description: 'Conditions that must be met in the webhook payload',
